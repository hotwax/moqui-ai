...
| 19 | Audit redaction | The `AiToolCall` row records the arguments the backing service ran with (filtered to the exposed schema, `fixed` applied), not the raw client input, and every value under a secret-named key (a built-in list; a deployment can add names through `ai_audit_redact_pattern` but never remove them) is stored as `***redacted***` in arguments and result, refused calls included. Tools take and return credentials, and the row is plain text that outlives the call. Audit-only: the service runs with, and the client receives, the real values. Agent-run rows and `AiToolCallRequest.arguments` are masked the same way (`org.moqui.ai.AuditRedactor`). |
